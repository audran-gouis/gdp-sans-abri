...
Then('je devrais voir le contenu des statistiques', async function() {
  const isVisible = await this.page.isVisible('#statistiques-tab.active');
  expect(isVisible).toBeTruthy();
  console.log('✅ Contenu des statistiques visible');
});

Then('l\'onglet {string} ne devrait plus être actif', async function(onglet) {
  const ongletMap = {
    'Transmissions Quotidiennes': 'transmissions',
    'ADP': 'adp',
    'Statistiques': 'statistiques'
  ...
  expect(!buttonActive && !contentActive).toBeTruthy();
  console.log(`✅ Onglet "${onglet}" n'est plus actif`);
});
