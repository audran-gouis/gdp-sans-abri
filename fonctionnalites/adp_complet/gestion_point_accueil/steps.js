...
When('je remplis les informations de base', async function() {
  await this.page.fill('#adp-form-nom', 'Test');
  await this.page.fill('#adp-form-prenom', 'User');
  await this.page.selectOption('#adp-form-typologie', { label: 'Homme Seul' });
  await this.page.selectOption('#adp-form-nb-personnes', '1');
});
...
Then('une nouvelle carte ADP avec Point Accueil devrait apparaître', async function() {
  ...
  // Vérifier que la carte contient "Point Accueil"
  let foundPointAccueil = false;
  for (const carte of cartes) {
    const contenu = await carte.textContent();
    if (contenu.includes('Point Accueil')) {
      foundPointAccueil = true;
      break;
    }
  }
  expect(foundPointAccueil).toBeTruthy();
});

Then('la carte devrait indiquer {string} actif', async function(badge) {
  await this.page.waitForTimeout(1000);
  ...
  // Vérifier que la carte contient le badge demandé
  const cartes = await this.page.$$('#adp-list > *');
  expect(cartes.length).toBeGreaterThan(0);
  
  // Debug: afficher le contenu de toutes les cartes
  for (let i = 0; i < cartes.length; i++) {
    const contenu = await cartes[i].textContent();
    console.log(`📋 Carte ${i + 1}:`, contenu.substring(0, 100));
  }
  
  let foundBadge = false;
  for (const carte of cartes) {
    const contenu = await carte.textContent();
    if (contenu.includes(badge)) {
      foundBadge = true;
      console.log(`✅ Badge "${badge}" trouvé dans la carte`);
      break;
    }
  }
  
  if (!foundBadge) {
    console.error(`❌ Badge "${badge}" non trouvé dans aucune carte`);
  }
